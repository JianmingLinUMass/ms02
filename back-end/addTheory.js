...
// file used locally for loading data into the theory.db

//the data to add, can be expanded when needed
const presentSimple = [
...
//function for looping through the array of data modules and adding them to the database
async function addBlocks(blocksToAdd) {
    try {
        ...
        for (const b of blocksToAdd) {
            ...
            await db.addTheoryBlock(text, unit, block);  //uses method from the database.js to add an entry to the database table
...
//array of theory units
allBlocks = [presentSimple];

//call function on the array 
allBlocks.forEach(i => {
    addBlocks(i)
});